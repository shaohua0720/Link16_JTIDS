...
int32_t encoder_Link16(int32_t n, int32_t m, string& inputs) {
	...
	string bit_data = Tools::stringToBitString(inputs);
	std::cout << "raw_data = " << inputs << std::endl;
	std::cout << "bit_data = " << bit_data << std::endl;

	//判断文件是否存在，若存在则删除
	if (!Tools::deleteFile("data.txt")) {
		return -2;
	}

	bitset<15> STN = bitset<15>(Tools::generateRandomBinary(15));
...
int32_t decoder_Link16(string& raw_data, int32_t& n, int32_t& m) {
	//从文件中读入数据
	string RT_bit_data = Tools::readMessage("data.txt");
	string bit_msg;
	...
	while (RT_bit_data.length() != 0) {
		...
		string str_group = RT_bit_data;
		RT_bit_data.clear();
		stdp_msg.setRawMsg(str_group);

		//解交织
		// TODO: 实现decode_weave函数
		// string str_weave = Tools::decode_weave(str_group);
		string str_weave = str_group;

		//解RS编码、AES解密、BIP校验
		// TODO: 实现decode_RS_AES_BIP_handler函数
		// string bit_data = Tools::decode_RS_AES_BIP_handler(str_weave);
		string bit_data = str_weave;
		if (bit_data.empty()) {
